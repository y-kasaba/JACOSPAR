FUNCTION CREATE_SYNTHETIC_SPECTRA_Limb_shoheiDatabase_onlydust_layercenter, orbit, $
  alt_range, $
  wn, $
  z, $
  ztpn, $
  ...
  FOR i = 0, nwindows - 1L DO BEGIN
    ...
    ENDIF ELSE BEGIN
      temp = CREATE_JACOSPAR_INPUT_FILES_Limb_shoheidatabase_onlydust_layercenter(orbit, wnrange(*, i) + [-FBord, FBord], gas_species, aerosols_species, alt_range, dwn, FOV, z, ztpn, Ht, geometry, Radiusm, Radiuss, wn_albedo, S_albedo, $
  ...
  IF numericaljacobian THEN BEGIN
    ...
    FOR i = 0, nwindows - 1L DO BEGIN
      ...
      indwn = indwn + nwntemp;indwn + nwntemp - 1L
    ENDFOR
  ENDIF ELSE BEGIN
    layer_bounds = GET_LAYER_BOUNDS_FROM_CENTERS(z)
    dz = (layer_bounds(1L : nlayer) - layer_bounds(0L : nlayer - 1L)) * 1D5
    ; Keep the top layer consistent with the original scale-height integration.
    dz(nlayer - 1L) = Ht(nlayer - 1L) * 1D5
    ...
    FOR i = 0, nwindows - 1L DO BEGIN
      ...
      openw,lun,path_output + orbit + nameplusdir + '/' +'Rtemp.txt',/get_lun
      data=fltarr(2,nwn)
      data(0,*)=wn & data(1,*)=transpose(Rtemp(*,0))
      printf,lun,data
      close,lun
      free_lun,lun
;----------------------kogure end --------------
      ...
      IF ~nojacobian THEN BEGIN
        ...
        FOR k = 0, ngas_fitted - 1L DO BEGIN
          ...
          FOR ii = 0, nspectra - 1L DO BEGIN
            FOR j = 0, nlayer - 1L DO Jactemp(*, j, ii, k + 1L) = dRdtauabstemp(*, j, ii) * dz(j) * ACSloc(*, j)
            FOR j = 0, nlayer - 1L DO dJactemp(*, j, ii, k + 1L) = deltadRdtauabstemp(*, j, ii) * dz(j) * ACSloc(*, j)
          ENDFOR
        ENDFOR
        ...
        FOR k = 0, naerosols_fitted - 1L DO BEGIN
          ...
          FOR ii = 0, nspectra - 1L DO BEGIN
            ; VMR
            FOR j = 0, nlayer - 1L DO Jactemp(*, j, ii, ngas_fitted + k + 1L) = (dRdtauabstemp(*, j, ii) * dz(j) * abscoeff(*, j) + $
              dRdtauscatemp(*, indloc, j, ii) * dz(j) * scacoeff(*, j))
            FOR j = 0, nlayer - 1L DO dJactemp(*, j, ii, ngas_fitted + k + 1L) = (deltadRdtauabstemp(*, j, ii) * dz(j) * abscoeff(*, j) + $
              deltadRdtauscatemp(*, indloc, j, ii) * dz(j) * scacoeff(*, j))

            ;Aerosols mean radius
            FOR j = 0, nlayer - 1L DO Jactemp(*, j, ii, ngas_fitted + naerosols_fitted + k + 1L) = (dRdtauabstemp(*, j, ii) * dz(j) * dabscoeffdrm(*, j) + $
              dRdtauscatemp(*, indloc, j, ii) * dz(j) * dscacoeffdrm(*, j))
            FOR j = 0, nlayer - 1L DO dJactemp(*, j, ii, ngas_fitted + naerosols_fitted + k + 1L) = (deltadRdtauabstemp(*, j, ii) * dz(j) * dabscoeffdrm(*, j) + $
              deltadRdtauscatemp(*, indloc, j, ii) * dz(j) * dscacoeffdrm(*, j))
            ...
            ; Aerosols variance radius
            FOR j = 0, nlayer - 1L DO Jactemp(*, j, ii, ngas_fitted + 2 * naerosols_fitted + k + 1L) = (dRdtauabstemp(*, j, ii) * dz(j) * dabscoeffds(*, j) + $
              dRdtauscatemp(*, indloc, j, ii) * dz(j) * dscacoeffds(*, j))
            FOR j = 0, nlayer - 1L DO dJactemp(*, j, ii, ngas_fitted + 2 * naerosols_fitted + k + 1L) = (deltadRdtauabstemp(*, j, ii) * dz(j) * dabscoeffds(*, j) + $
              deltadRdtauscatemp(*, indloc, j, ii) * dz(j) * dscacoeffds(*, j))
          ENDFOR
        ENDFOR
