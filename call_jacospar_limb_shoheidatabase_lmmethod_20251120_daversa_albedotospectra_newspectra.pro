FUNCTION CALL_JACOSPAR_LIMB_shoheidatabase_LMmethod_20251120_Daversa_AlbedoToSpectra_newSpectra, orbit, $
  alt_range, $
  ...
  printf,lun,"iter","kai2(iter)","convX(iter)","rho","v"
  ...
  WHILE (iter LT itermax) AND (convX(iter - 1) GE epsilonX) AND ~nofit DO BEGIN;;;;20211109 KogureRisei.for L-M method.
    ...
    h = LA_INVERT(((1d + v)*invSa + TRANSPOSE(Kmat(*, *, iter - 1L)) # DIAG_MATRIX(invSe) # Kmat(*, *, iter - 1L)) , /DOUBLE, STATUS = status) # (TRANSPOSE(Kmat(*, *, iter - 1L)) # DIAG_MATRIX(invSe) # (Y - Fxk(iter - 1l,*)) - invSa # (reform(xk(iter-1,*)) - xa));20211101 KogureRisei. Levenberg - Marquardt Method[Inverse methods for atmospheric sounding. page92]
    xk(iter,*) = xk(iter-1,*) + h
    ;xk(iter,*) = xk(iter-1,*) + LA_INVERT(((1d + v)*invSa + TRANSPOSE(Kmat(*, *, iter - 1L)) # DIAG_MATRIX(invSe) # Kmat(*, *, iter - 1L)) , /DOUBLE, STATUS = status) # (TRANSPOSE(Kmat(*, *, iter - 1L)) # DIAG_MATRIX(invSe) # (Y - Fxk(iter - 1l,*)) - invSa # (reform(xk(iter-1,*)) - xa));20211101 KogureRisei. Levenberg - Marquardt Method[Inverse methods for atmospheric sounding. page92]
    ...
    ;;;phi chack
    r = y - Fxk(iter,*)
    phi_data(iter) = DOUBLE( TRANSPOSE(r) # DIAG_MATRIX(invSe) # r )

    PRINT, 'phi_data(iter)=', phi_data(iter), 'phi_data(iter-1)=', phi_data(iter-1)
    ...
    if rho gt 0 then begin
      ...
      ; ---- revert everything to previous (accepted) state ----
      xk(iter,*)          = xk(iter-1,*)
      ...
      ; ログ
      printf, lun, iter, kai2(iter), convX(iter), rho, v
      print, 'rejected: kai=', kai2(iter), 'rho=', rho, ', v=', v
    
      ; v 上限で中断（revert 済みなので安全）
