FUNCTION call_jacospar_limb_shoheidatabase_lmmethod_20250401_Daversa_AlbedoToSpectra_daversaSpectra, orbit, $
  alt_range, $
  ...
  DWN = dwn, $  ztpn_ini = GETZTPN_LimbTest_new_20240220_invsaTest(MEAN(geometry(4, *)), MEAN(geometry(5, *)), MEAN(geometry(6, *)), MEAN(geometry(7, *)), z, cov, StartRadius, appriori_files, cov_parameters)
  FACTDUST = factdust, $
  FACTORINI = factorini, $
  ...
  ; Read geometry
  ; -------------
  geometry = READ_GEOMETRY_Emiliano(orbit)
...
;  Radiusm(0,*,0) = interpol(Radius_ap(1,*), Radius_ap(0,*), z(0,*));;;;;;;2022/1/1 kogure risei 線形に粒径が減少するプロファイル
  ...
  IF usesynthetic THEN BEGIN
    ...
    drad_measured(*) = .01D
  ENDIF ELSE BEGIN
    rad_measured = READ_Emiliano_SPECTRA(orbit, WN = wn_measured, ERROR = drad_measured, _EXTRA = extra)
  ...
  FOR i = 0, nlayer - 1 DO BEGIN
    CASE i OF
      ...
      9: BEGIN
         ...
         END
      ELSE: BEGIN
               Radiusm[0, i, 0] = 0.65e-4  ; デフォルト値
            END
    ENDCASE
  ENDFOR
  ;Radiusm[0, *, 0] = Radiusm[0, *, 0] * 0.3D0  ; スケーリングファクター
...
FREE_LUN, lun_output
  ...
  WHILE (iter LT itermax) AND (convX(iter - 1) GE epsilonX) AND ~nofit OR (Ratio lt 0) DO BEGIN;;;;20211109 KogureRisei.for L-M method.
    ...
    if rho gt 0 then begin
      ...
      v = v*LM_update_rate
      if v gt 1e7 then v=1e7
      ;xk(iter,*) = xk(iter-1,*);
      ;ztpn(*,*,iter) = ztpn(*,*,iter-1)
      ;jac(*,*,*,*,iter)=jac(*,*,*,*,iter-1)
      ;Kmat(*, *, iter)=Kmat(*, *, iter - 1L)
      ;X(*,*,iter)=X(*,*,iter-1)
