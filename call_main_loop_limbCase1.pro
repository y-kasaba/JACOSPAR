...
PRO call_main_loop_limbCase1
  ...
  ;--------------------- ここまでは共通 --------------------------;
      ...
      ;----------------------- 感度実験1 ---------------------------;
    ;保存ディレクトリ名
    NAMEPLUSDIR = '20251118_dwn1_0-80km_limbCase3_gas_test1_dz=dz_ice2';NAMEPLUSDIR = '20240225_convergetest_1_LimbTestDust_1d-6_NadirTestH2Oice_1d-6'

    ;orbit number
    orbit = '4483'

    ;エアロゾルアプリオリのファイルパス
    filenamedust = 'NadirTest_dust.dat'
    filenameice = 'NadirTest_H2O_ice.dat';'NadirTest_H2O_ice.dat';;;;;phobos版のJACOSPAR同様、初めのリトリーバル感度をダストと同じにするために、ダストと同じアプリオリプロファイルを用いる。
    appriori_files = [filenamePT, filenameCO2, filenameCO, filenameH2O, filenamedust, filenameice, filename]

    ;エアロゾルアプリオリの粒径
    StartRadius = [1.5d-4, 2.0d-4]
    ...
    ;関数の呼び出し
    c = call_jacospar_limb_shoheidatabase_lmmethod_20240220_limbCase1(orbit, altitude, NAMEPLUSDIR = NAMEPLUSDIR, cov_parameters = cov_parameters,$
      ...
      StartRadius = StartRadius, appriori_files = appriori_files, LM_initial_v = LM_initial_v,LM_update_rate = LM_update_rate, $;dz = dz, $
      /plottotalopticaldepth, /nofov ,/MAKEPRINT,/fitradius,/dodependence) ;/makeplot
