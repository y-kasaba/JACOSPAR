FUNCTION READ_SPECTRA_2025, orbit, WN = wn, ERROR = error, DOERROR = doerror

  pathnames = PATH_MANAGEMENT()
  PathName = pathnames.spectra
  sdir = FILE_SEARCH(pathname + 'Spectra_' + STRMID(orbit, 0, 4) + "_kogure20260119.dat")
  ...
  IF KEYWORD_SET(doerror) THEN BEGIN
    sdir = FILE_SEARCH(pathname + 'Sigma_Emiliano_' + STRMID(orbit, 0, 4) + ".dat")
