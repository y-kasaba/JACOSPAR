FUNCTION UPDATE_JACOSPAR_INPUT_PARALLEL_Limb_ShoheiDatabase_limbCase1_mlos0, seq, $
  outputfile, $
  fov, $
  npf, $
  nang, $
...
;  thetaP(*) = 180d;;kogure to calculate nadir 2020/5/28 theP
  ...
  plos = 90 + thetaP; V（測器）におけるVP（観測点）ベクトルの天頂角
  ;  dphi = ABS(geom(2, *))
...
;  thesol = geom(3, *) 
  ...
  FOR i = 0, nspectra - 1 DO BEGIN
    ...
    IF ~onlynames THEN BEGIN
      ...
      PRINTF, lun, "  mlos = 0         ! method for defining plos (0:theV, 1:theP, 2:angPOV, 3:hgtH)";PRINTF, lun, "  mlos = 2         ! method for defining plos (0:theV, 1:theP, 2:angPOV, 3:hgtH)";;2021/4/19
