FUNCTION UPDATE_JACOSPAR_INPUT_PARALLEL_Limb_ShoheiDatabase_limbCase1_mlos1, seq, $
  outputfile, $
  fov, $
  npf, $
  nang, $
...
;  thetaP = ACOS((zobs + Rmars) / (hd + Rmars)) / !DPI * 180D ; angle VOP;;2021/4/21 kogure
  plos = dblarr(n_elements(geom(1, *)))
  plos(*) = 90; V（測器）におけるVP（観測点）ベクトルの天頂角
  ;  dphi = ABS(geom(2, *))
...
;  thesol = geom(3, *) 
  ...
  FOR i = 0, nspectra - 1 DO BEGIN
    ...
    IF ~onlynames THEN BEGIN
      ...
      PRINTF, lun, "  mlos = 1         ! method for defining plos (0:theV, 1:theP, 2:angPOV, 3:hgtH)";PRINTF, lun, "  mlos = 2         ! method for defining plos (0:theV, 1:theP, 2:angPOV, 3:hgtH)";;2021/4/19
      PRINTF, lun, "  plos = " + STRING(plos(i), FORMAT = "(F11.7)") + " ! theP"
      PRINTF, lun, "  mgeoH = 1        ! tangent point geometry, 1:forward(VPH/PVH), -1:backward(PHV/VHP)"
      PRINTF, lun, "  phiobs = " + STRING(0D, FORMAT = "(F11.7)") + " ! azimuth angle (deg.) of V-to-P vector";;;;;;;;;;;;;;;;;;;;;;change to nadir parameter  original =180d
