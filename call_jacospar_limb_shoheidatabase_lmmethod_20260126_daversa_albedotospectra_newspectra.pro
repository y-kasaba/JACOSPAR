FUNCTION CALL_JACOSPAR_LIMB_shoheidatabase_LMmethod_20260126_Daversa_AlbedoToSpectra_newSpectra, orbit, $
  alt_range, $
  ...
  APPRIORI_FILES = appriori_files,$
  LM_initial_v = LM_initial_v,$
  LM_update_rate = LM_update_rate,$
  ;LM_v_limit = LM_v_limit,$
  ...
  ;;;;;;; ver-20241012 : update to RADIUSPREFIT_shoheidatabase_onlyRadius
  ; -----------------------
  ...
  IF ~KEYWORD_SET(threshold) THEN threshold = 1D-4;  20240316 kogure change to 1d-4. 1d-3, 1d-4のときの収束したプロファイルを比較し、1d-4が妥当と判断。
  resol = 31.2461D
  ...
  ; Read geometry
  ; -------------
  geometry = READ_GEOMETRY_2025(orbit)
  ...
  ; -------------------
  ; Calculating the ILS　OMEGAのInstrument Line Shape
  ; -------------------
  ...
  IF usesynthetic THEN BEGIN
    ...
    drad_measured(*) = .01D
  ENDIF ELSE BEGIN
    rad_measured = READ_SPECTRA_2025(orbit, WN = wn_measured, ERROR = drad_measured, _EXTRA = extra)
  ...
  ztpn_ini = GETZTPN_LimbTest_new_20240220_invsaTest(MEAN(geometry(4, *)), MEAN(geometry(5, *)), MEAN(geometry(6, *)), MEAN(geometry(7, *)), z, cov, StartRadius, appriori_files, cov_parameters);T[K],P[pa],CO2ndens,,
  ...
  ; ファイルの読み込み（1行目のヘッダーをスキップ）
  albedoText = READ_ascii(pathnames.albedo + 'limb_0072_2_surf_alb_jun_19.txt', DATA_START=1)

  ; データ行の文字列配列を取得
  lines = albedoText.FIELD1
  ...
  albedo_values = FLTARR(n)

  ; 偶数番目が波長、奇数番目がアルベド
  FOR i = 0, n - 1 DO BEGIN
    wavelength_nm[i] = lines[2*i]
    albedo_values[i] = lines[2*i + 1]
  ENDFOR
  ...
  ; 並び替えのインデックスを取得（昇順）
  sort_index = SORT(wavenumber)

  ; 並び替えた配列を作成
  wavenumber_sorted = wavenumber[sort_index]
  albedo_sorted = albedo_values[sort_index]

  ; 補間
  S_albedo = DBLARR(nwn, itermax)
  S_albedo[*, 0] = INTERPOL(albedo_sorted, wavenumber_sorted, wn_albedo)
  ...
  ;S_albedo(*,0) = 0.40d;makearr(n_elements(S_albedo(*,0)),Value=0.1500000)
  ; --------------------
  ; Prefit of the radius
  ; --------------------
  IF doradiusprefit THEN BEGIN
    resultsprefit = RADIUSPREFIT_shoheidatabase_onlyRadius(orbit, alt_range, ztpn(*, *, 0), geometry, FOV, wnall, radall, _EXTRA = extra)
    Radiusm(0, *, 0) = resultsprefit(*, 0)
    ;    Radiuss(0, *, 0) = resultsprefit(*, 1)
  ENDIF
  ;;;;;;;;;;;;;;;;;;;; Radius from Daversa
  ; Radiusmの2次元目のサイズ（shape(radiusm(0,*,0))）を取得し、そのサイズに合った数だけ以下を挿入する
  FOR i = 0, nlayer - 1 DO BEGIN
    CASE i OF
      0: BEGIN
           Radiusm[0, i, 0] = 1.73e-4
         END
      1: BEGIN
           Radiusm[0, i, 0] = 1.73e-4
         END
      2: BEGIN
           Radiusm[0, i, 0] = 1.37e-4
         END
      3: BEGIN
           Radiusm[0, i, 0] = 0.9e-4
         END
      4: BEGIN
           Radiusm[0, i, 0] = 0.92e-4
         END
      5: BEGIN
           Radiusm[0, i, 0] = 0.77e-4
         END
      6: BEGIN
           Radiusm[0, i, 0] = 0.79e-4
         END
      7: BEGIN
           Radiusm[0, i, 0] = 0.71e-4
         END
      8: BEGIN
           Radiusm[0, i, 0] = 0.65e-4
         END
      9: BEGIN
           Radiusm[0, i, 0] = 0.65e-4
         END
      ELSE: BEGIN
               Radiusm[0, i, 0] = 0.4e-4  ; デフォルト値
            END
    ENDCASE
  ENDFOR
  ; -----------------------
  ...
  ; -----------------------
  radtemp = CREATE_SYNTHETIC_SPECTRA_Limb_shoheiDatabase_onlydust(orbit, alt_range, wn_apriori, z, ztpn(*, *, 0), Ht, geometry, dwn, FOV, Radiusm(*, *, 0), $
  ...
  ; ---- DEBUG: grid / flags sanity ----
  print, '--- DEBUG interp gate ---'
  print, 'nomeasinterp=', nomeasinterp, ' checkjacobiank=', checkjacobiank
  print, 'N(wn_apriori)=', N_ELEMENTS(wn_apriori), '  wn_apriori[0,-1]=', wn_apriori[0], wn_apriori[-1]
  print, 'N(wn_measured)=', N_ELEMENTS(wn_measured), '  wn_measured[0,-1]=', wn_measured[0], wn_measured[-1]
  ...
  ENDIF ELSE BEGIN
    ...
    FOR i = 0, nspectra - 1 DO BEGIN
      ...
      IF i EQ 0 THEN BEGIN
        print, '--- DEBUG after interpol (i=0) ---'
        print, 'N(radtemp)=', N_ELEMENTS(radtemp(*,0)), '  N(rad)=', N_ELEMENTS(rad(*,0,0))
        print, 'radtemp[0],radtemp[-1]=', radtemp[0,0], radtemp[-1,0]
        print, 'rad[0],rad[-1]=', rad[0,0,0], rad[-1,0,0]
        print, 'wn_measured sample:', wn_measured[0], wn_measured[1], wn_measured[2], ' ... ', wn_measured[-1]
      ENDIF
    ENDFOR
  ENDELSE
  ...
  wvl=lambda_measured
  alt=geometry(0, *)
  save,radiance,wvl,alt,filename = pathnames.results + orbit + NAMEPLUSDIR + '/Rad_orb'+orbit+'.sav'
  ;=======================
  ...
  invSa = GET_INVSA_invsaTest(X(*, *, 0), z, gas_species_all, aerosols_species_all, onlyfitspecies, corrlength, fitradius, fitsigma, Radiusm(*, *, 0), Radiuss(*, *, 0), cov, S_albedo(*, 0), wn_albedo, MEAN(wn_measured(1 : nwn - 1L) - wn_measured(0 : nwn - 2L)), _EXTRA = extra);アプリオリの分散・この中でSaを変えている。
  ...
  xk(0, *) = xa
  iter = 1L
  v = LM_initial_v;20211101 KogureRisei. Parameter of Levenberg - Marquardt Method
  ...
  IF N_ELEMENTS(invSa) GT 1 THEN Sy = DIAG_MATRIX(1d/invse) # (1d/(Kmat(*, *, 0) # LA_INVERT(invSa, /DOUBLE, STATUS = status) # transpose(Kmat(*, *, 0)) + DIAG_MATRIX(1d/invse))) # DIAG_MATRIX(1d/invse) else Sy = DIAG_MATRIX(1d/invse) # (1d/(Kmat(*, *, 0) # (1d/invSa) # transpose(Kmat(*, *, 0)) + DIAG_MATRIX(1d/invse))) # DIAG_MATRIX(1d/invse) ;20211119 kogure risei. rodger 5.27式
  IF N_ELEMENTS(Sy) GT 1 THEN kai2(0) = ABS(transpose(y - Fxk(0, *)) # La_INVERT(Sy, /DOUBLE, STATUS = status) # (y - Fxk(0, *))) else kai2(0) = ABS(transpose(y - Fxk(0, *)) # (1d/Sy) # (y - Fxk(0, *)));;20211119 kogure risei 評価関数のため加えた. rodger 5.32式
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ; Inserted code to output Se, Sa, and related variables for debugging purposes
  ; Assuming invSe, invSa, and other variables are arrays or matrices

  ; Define the output file path
  output_file = PathSave + 'variables_debug_output.txt'

  ; Open file for writing
  OPENW, lun_output, output_file, /GET_LUN

  ; Write header
  PRINTF, lun_output, 'Debug Output for Variables:'
  PRINTF, lun_output, '============================'

  ; Write invSe to the file
  PRINTF, lun_output, 'invSe (Observation Error Matrix Inverse):'
  IF N_ELEMENTS(invSe) GT 0 THEN BEGIN
    FOR i = 0, N_ELEMENTS(invSe) - 1 DO BEGIN
      PRINTF, lun_output, STRING(invSe[i])
    ENDFOR
  ENDIF ELSE PRINTF, lun_output, 'invSe is not initialized or empty.'

  ; Write invSa to the file
  PRINTF, lun_output, '\ninvSa (A Priori Covariance Matrix Inverse):'
  IF N_ELEMENTS(invSa) GT 0 THEN BEGIN
    FOR i = 0, N_ELEMENTS(invSa) - 1 DO BEGIN
      PRINTF, lun_output, STRING(invSa[i])
    ENDFOR
  ENDIF ELSE PRINTF, lun_output, 'invSa is not initialized or empty.'

  ; Write Kmat dimensions and sliced sample values to the file
  PRINTF, lun_output, '\nKmat (Jacobian Matrix):'
  IF N_ELEMENTS(Kmat) GT 0 THEN BEGIN
    dims = SIZE(Kmat, /DIMENSIONS)
    PRINTF, lun_output, 'Kmat dimensions:', STRING(dims)
    PRINTF, lun_output, '--- Kmat (Layer 1, All Spectra, Iteration 0) ---'
    FOR i = 0, dims[1] - 1 DO BEGIN
      PRINTF, lun_output, STRING(Kmat[0:*, i, 0])
    ENDFOR
    PRINTF, lun_output, '--- Kmat (Layer 1, Wavelength 1, Iteration 0) ---'
    PRINTF, lun_output, STRING(Kmat[0, 0, 0])
  ENDIF ELSE PRINTF, lun_output, 'Kmat is not initialized or empty.'

  ; Write X and xa to the file
  PRINTF, lun_output, '\nX (Retrieved State Vector):'
  IF N_ELEMENTS(X) GT 0 THEN BEGIN
    FOR i = 0, N_ELEMENTS(X) - 1 DO BEGIN
      PRINTF, lun_output, STRING(X[i])
    ENDFOR
  ENDIF ELSE PRINTF, lun_output, 'X is not initialized or empty.'

  PRINTF, lun_output, '\nxa (A Priori State Vector):'
  IF N_ELEMENTS(xa) GT 0 THEN BEGIN
    FOR i = 0, N_ELEMENTS(xa) - 1 DO BEGIN
      PRINTF, lun_output, STRING(xa[i])
    ENDFOR
  ENDIF ELSE PRINTF, lun_output, 'xa is not initialized or empty.'

  ; Write residual to the file
  PRINTF, lun_output, '\nResidual (Y - Fxk):'
  IF N_ELEMENTS(Y) GT 0 AND N_ELEMENTS(Fxk) GT 0 THEN BEGIN
    FOR i = 0, N_ELEMENTS(Y) - 1 DO BEGIN
      PRINTF, lun_output, STRING(Y[i] - Fxk[i])
    ENDFOR
  ENDIF ELSE PRINTF, lun_output, 'Residual cannot be calculated because Y or Fxk is not initialized or empty.'

  ; Write additional variables if needed (example)
  PRINTF, lun_output, '\nOther Variables:'
  PRINTF, lun_output, '------------------'
  ; Add more variables here as needed, e.g.,
  ; PRINTF, lun_output, 'VariableName: ', STRING(Variable)
  ...
  openw,lun,path_output+'levenbergMarquardt',/get_lun
  printf,lun,"iter","phi(iter)","convX(iter)","v"
  
  phi_data  = DBLARR(itermax)
  phi_prior = DBLARR(itermax)
  phi       = DBLARR(itermax)

  ; --- iter=0 の phi を入れる ---
  r0 = y - Fxk(0,*)
  phi_data(0)  = TOTAL((r0^2) * invSe)            ; invSe が 1/sigma^2 ベクトルなら最速
  dx0 = reform(xk(0,*)) - xa
  IF N_ELEMENTS(invSa) GT 1 THEN phi_prior(0) = TRANSPOSE(dx0) # invSa # dx0 $
  ELSE phi_prior(0) = TOTAL(dx0^2) * invSa

  phi(0)       = phi_data(0) + phi_prior(0)
  
  ; ループの外で初期化
  trial = 0L

  WHILE (iter LT itermax) AND (convX(iter - 1) GE epsilonX) AND ~nofit DO BEGIN;;;;20211109 KogureRisei.for L-M method.
    IF makeprint THEN PRINT, "Iter " + STRING(iter, "(I02)") + ":"
    
    trial = trial + 1L
    name_try = 'step' + STRING(iter,'(I02)') + '_t' + STRING(trial,'(I02)') + '_'
    ...
    HESS = invSa + TRANSPOSE(Kmat(*,*,iter-1)) # DIAG_MATRIX(invSe) # Kmat(*,*,iter-1)
    gvec = TRANSPOSE(Kmat(*,*,iter-1)) # DIAG_MATRIX(invSe) # (Y - Fxk(iter-1,*)) - invSa # (reform(xk(iter-1,*)) - xa)
    ; damping matrix (diag(HESS))
    n = N_ELEMENTS(xa)
    II = DBLARR(n,n)
    FOR i=0, n-1 DO II(i,i) = HESS(i,i)
    h = LA_INVERT(Hess + v*II, /DOUBLE, STATUS=status) # gvec
    xk(iter,*) = xk(iter-1,*) + h
    print, 'max|h|=', MAX(ABS(h)), '  max|dx|=', MAX(ABS(xk(iter,*)-xk(iter-1,*)))
    ...
    print, 'max|dztpn|=', MAX(ABS(ztpn(*,*,iter)-ztpn(*,*,iter-1)))
    ...
    radtemp = CREATE_SYNTHETIC_SPECTRA_Limb_shoheiDatabase_onlydust(orbit, alt_range, wn_apriori, z, ztpn(*, *, iter), Ht, geometry, dwn, FOV, Radiusm(*, *, iter), Radiuss(*, *, iter), wn_albedo, S_albedo(*, iter), jactemp, $
      ILS = ILS, NAMEPLUSFILE = name_try, _EXTRA = extra)
    FOR i = 0, nspectra - 1 DO BEGIN
    ...
    IF N_ELEMENTS(Sy) GT 1 THEN kai2(iter) = ABS(transpose(y - Fxk(iter, *)) # La_INVERT(Sy, /DOUBLE, STATUS = status) # (y - Fxk(iter, *))) else kai2(iter) = ABS(transpose(y - Fxk(iter, *)) # (1d/Sy) # (y - Fxk(iter, *)));;20211119 kogure risei 評価関数のため加えた. rodger 5.32式
    
    ; ---- Rodgers cost function phi ----
    r = y - Fxk(iter,*)
    phi_data(iter) = TOTAL((r^2) * invSe)
    
    dx = reform(xk(iter,*)) - xa
    IF N_ELEMENTS(invSa) GT 1 THEN phi_prior(iter) = TRANSPOSE(dx) # invSa # dx $
    ELSE phi_prior(iter) = TOTAL(dx^2) * invSa    
    phi(iter) = phi_data(iter) + phi_prior(iter)
    
    PRINT, 'phi(iter)=', phi(iter), 'phi_prev=', phi(iter-1), $
           'phi_data=', phi_data(iter), 'phi_prior=', phi_prior(iter),$
           'dphi=', phi(iter)-phi(iter-1)
    ...
    ;;;The Levenberg-Marquardt algorithm for nonlinear least squares curve-fitting problems, Henri 2024. Equetion(15)
    Jacobian = GET_J(jac(*, *, *, *, iter), nspectra, regions, onlyfitspecies, fitradius, fitsigma, nlayer, fitalbedo)
    ;rho = (kai2(iter-1)-kai2(iter))/ABS(transpose(h) # (v*h + transpose(transpose(Jacobian) # La_INVERT(Sy, /DOUBLE, STATUS = status) # (y - Fxk(iter-1, *)))))
    ;print,'kai2(iter)=', kai2(iter) ,'kai2(iter-1)=', kai2(iter-1) ,ABS(transpose(h) # (v*h + transpose(transpose(Jacobian) # La_INVERT(Sy, /DOUBLE, STATUS = status) # (y - Fxk(iter-1, *)))))
    ;print,'rho',rho
    ;
    print, 'Fxk diff max = ', MAX(ABS(Fxk(iter,*) - Fxk(iter-1,*)))
    print, '||K||_max = ', MAX(ABS(Kmat(*,*,iter)))
    print, 'N(y),N(invSe)=', N_ELEMENTS(y), N_ELEMENTS(invSe)
    print, '||h||1 = ', TOTAL(ABS(h))
    print, 'v=', v, '  phi_data_delta=', phi_data(iter)-phi_data(iter-1), '  phi_prior_delta=', phi_prior(iter)-phi_prior(iter-1)


    ;
    ;;;The Levenberg-Marquardt algorithm for nonlinear least squares curve-fitting problems, Henri 2024. Chapter-4.1.1, option-1
    ;;;更新が線形の更新よりも良い結果だったら更新をacceptし、ラムダ(v)を小さくする。そうでなければ、更新は受け入れず、ラムダ（v）は大きくする。
    if phi(iter) lt phi(iter-1) then begin
      ; accept
      v = v/LM_update_rate
      if v lt 1e-7 then v = 1e-7
    endif else begin
      ; reject
      v = v*LM_update_rate
      ...
      ; ---- revert everything ----
      xk(iter,*)        = xk(iter-1,*)
      ztpn(*,*,iter)    = ztpn(*,*,iter-1)
      Kmat(*,*,iter)    = Kmat(*,*,iter-1)
      Fxk(iter,*)       = Fxk(iter-1,*)
      X(*,*,iter)       = X(*,*,iter-1)
      Radiusm(*,*,iter) = Radiusm(*,*,iter-1)
      Radiuss(*,*,iter) = Radiuss(*,*,iter-1)
      S_albedo(*,iter)  = S_albedo(*,iter-1)
      rad(*,*,iter)     = rad(*,*,iter-1)
      jac(*,*,*,*,iter) = jac(*,*,*,*,iter-1)
      convX(iter)       = convX(iter-1)
      kai2(iter)        = kai2(iter-1)
      phi(iter)         = phi(iter-1)
      phi_data(iter)    = phi_data(iter-1)
      phi_prior(iter)   = phi_prior(iter-1)
      if makeplot then RMS(*,iter) = RMS(*,iter-1)
    
      printf, lun, iter, phi(iter), convX(iter), -1.0, v
      print, 'rejected: phi=', phi(iter), ', v=', v
    
      if v ge 1e7 then begin
        print, 'LM: v hit upper limit; aborting iteration.'
        nofit = 1
        break
      endif
    ...
    ;if ratio ge 0 then v = v/LM_update_rate;20211101 KogureRisei.for L-M method.
    ;if ratio lt 0 then v = v*LM_update_rate ;20211101 KogureRisei.for L-M method.
    ;if v gt LM_v_limit then v = LM_v_limit
    ;if v lt 800d then v = 800
    ;print,'updated: kai=', kai2(iter) , 'rho=',rho,', v=', v;20211101 KogureRisei.for L-M method.
  ...
  ENDIF
  IF makeprint THEN BEGIN
    IF nofit THEN PRINT, "Inversion aborted (LM damping hit limit)."
