FUNCTION CREATE_JACOSPAR_INPUT_FILES_Limb_ShoheiDatabase_onlydust_layercenter, orbit, $
  wnrange, $
  gas_species, $
  aerosols_species, $
  alt_range, $
  ...
  dz = layer_bounds(1L : nlayer) - layer_bounds(0L : nlayer - 1L)
  ; Keep the top layer consistent with the original scale-height integration.
  dz(nlayer - 1L) = Ht(nlayer - 1L)
  nu_ref = 8500D
  ...
  IF ngas GT 0 THEN BEGIN
    ...
    FOR i = 0, ngas - 1 DO BEGIN
      CASE STRLOWCASE(gas_species(i)) OF
        ...
        'h2o': column = 7
      ENDCASE
      FOR j = 0, nlayer - 1L DO tau_gas_abs(j, *, i) = Gas_ACS(*, j, i) * ztpn(column, j) * dz(j) * 1D3
    ENDFOR
    DELVAR, column
  ...
  for j = 0, nlayer - 1 DO BEGIN
    IF ztpn(8, j) GT 0 THEN BEGIN
      phs_dustpre=read_ascii(pathnames.aerosols_phs + radiuspath(0,j) + '_phs.dat',COMMENT_SYMBOL='%');,data_start=1)
      phs_dustpre=phs_dustpre.field001
      phs_dust=fltarr(182,10001)
      phs_ice=fltarr(182,10001)
      ;phs_icepre=read_ascii(pathnames.aerosols_phs + radiuspath(1,j) + '_phs.dat',COMMENT_SYMBOL='%')
      ...
      for i=0,N_wn_aero-1 do begin
        near=Min((1d/wn_phs*1d4 - wnaero(i))^2, index)
        wnind(i)=index
      endfor
  ...
  FOR i = 0, naerosols - 1 DO BEGIN
    CASE STRLOWCASE(aerosols_species(i)) OF
      ...
      'waterice': column = 9
    ENDCASE
    FOR j = 0, nlayer - 1L DO BEGIN ; Aero_k_sca : cm2 ; ztpn(column,j): cm-3; layer thickness * 1e5: cm
      tau_aero_sca(j, *, i) = Aero_k_sca(*, i, j) * ztpn(column, j) * dz(j) * 1D5
      tau_aero_abs(j, *, i) = Aero_k_abs(*, i, j) * ztpn(column, j) * dz(j) * 1D5
    ENDFOR
  ENDFOR
  DELVAR, column
  ...
  IF plottotalopticaldepth AND KEYWORD_SET(makeplot) THEN BEGIN
  ...
  ; -----------------------------------------
  sequence = LINDGEN(nspectra)
  FileNames = UPDATE_JACOSPAR_INPUT_PARALLEL_Limb_ShoheiDatabase_layercenter(sequence, savename, FOV,naerosols * (N_wn_aero - 1), n_angles, precision, seed, ngroups, nmembers, naerosols, ngas, path_output, alt_range, z, Ht, GEOMETRY = geometry, _EXTRA = extra)
