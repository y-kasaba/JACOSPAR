FUNCTION CREATE_JACOSPAR_INPUT_FILES_Limb_ShoheiDatabase_limbCase1, orbit, $
  wnrange, $
  gas_species, $
  aerosols_species, $
  alt_range, $
  ...
  IF ~KEYWORD_SET(makeplot) THEN makeplot = 0
...
;  free_lun,lun
  ...
  IF plottotalopticaldepth and makeplot THEN BEGIN
  ...
  ; -----------------------------------------
  FileNames = UPDATE_JACOSPAR_INPUT_PARALLEL_Limb_ShoheiDatabase_limbCase1_mlos3(sequence, savename, FOV,naerosols * (N_wn_aero - 1), n_angles, precision, seed, ngroups, nmembers, naerosols, ngas, path_output, alt_range, z, GEOMETRY = geometry, _EXTRA = extra) ; dz
