FUNCTION UPDATE_JACOSPAR_INPUT_PARALLEL_Limb_ShoheiDatabase_layercenter, seq, $
  outputfile, $
  fov, $
  npf, $
  nang, $
  ...
  ; Keep the top layer boundary consistent with the scale-height integration.
  layer_bounds(nlayer) = layer_bounds(nlayer - 1L) + Ht(nlayer - 1L)
...
;    thetaP(*) = 180d;;kogure to calculate nadir 2020/5/28 theP
  ...
  dphi = ABS(geom(2, *))
;  phisol_abs = geom(11, *)       ;20260226 kogure Sun→P 絶対方位角 [deg] (0=south)
...
;    thesol(*)=30d;zenith
  ...
  FOR i = 0, nspectra - 1 DO BEGIN
    ...
    IF ~onlynames THEN BEGIN
      ...
      PRINTF, lun, "  hbot = " + STRING(layer_bounds(0) * 1D3, "(F7.1)") + " ! altitude (m) of bottom of atmosphere (BOA)"
      stringalt = ''
      FOR j = 1, N_ELEMENTS(layer_bounds) - 2 DO stringalt = stringalt + STRING(layer_bounds(j) * 1D3, "(F7.1)") + ", "
      stringalt += STRING(layer_bounds(nlayer) * 1D3, "(F7.1)")
