FUNCTION READ_Emiliano_SPECTRA, orbit, WN = wn, ERROR = error, DOERROR = doerror

  pathnames = PATH_MANAGEMENT()
  PathName = pathnames.spectra
  sdir = FILE_SEARCH(pathname + 'Converted_Spectra_' + STRMID(orbit, 0, 4) + ".dat")
...
;  spectra = spectra.FIELD01
  ...
  IF KEYWORD_SET(doerror) THEN BEGIN
    sdir = FILE_SEARCH(pathname + 'Sigma_Emiliano_high_' + STRMID(orbit, 0, 4) + ".dat")
...
;    error = error.FIELD1
 ;   error = error.FIELD01
  ...
  ENDIF ELSE BEGIN
    ...
    error[WHERE(error LT 2d-3)] = 2d-3 ; Add this line. Risei Kogure 2021/10/8
  ENDELSE
  RETURN, spectra
END